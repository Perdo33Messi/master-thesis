...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'franka_pick_dyn_front_u_static_groove_obstacles.xml')
...
# FrankaFetchPickDynFrontUStaticGrooveEnv
# class FrankaFetchPickDynObstaclesEnv(robot_env.RobotEnv, gym.utils.EzPickle):
class FrankaFetchPickDynFrontUStaticGrooveEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=20):
        ...
        # self.field = [1.23, 0.75, 0.6, 0.25, 0.35, 0.2]

        # ===========================
        # 现在是 "U" 形槽，共有三块 geom：geom_u_left, geom_u_right, geom_u_bottom
        # ===========================
        ### U-shape：U 形的三块几何
        self.dyn_obstacles_geom_names = [
            'obstacle:geom',
            'obstacle2:geom_u_left',   # 左侧竖板
            'obstacle2:geom_u_right',  # 右侧竖板
            'obstacle2:geom_u_bottom'  # 底部横板
        ]
        ...
        # ---------------------------
        # dyn_obstacles 里存的是障碍物中心、四元数、以及长宽高等信息
        # 第一个是小蓝方块（obstacle）
        # 第二个是“U”形整体的 body（由三个 geom 构成，但可用一个包络做整体）
        # ---------------------------
        self.dyn_obstacles = [
            [1.3, 0.60, 0.435, 1.0, 0.0, 0.0, 0.0, 0.03, 0.03, 0.03],  # obstacle（蓝方块）
            # [1.23, 0.60, 0.435, 1.0, 0.0, 0.0, 0.0, 0.03, 0.03, 0.03],  # obstacle（蓝方块）
            # 注意：即使 xml 里拆成三块 geom_u_left / geom_u_right / geom_u_bottom
            # 这里依旧当作一个整体 body 来处理，用一个包络大小即可
            ### CHANGED：注释中改为 U 形
            ### CHANGED: “U”形的外包尺寸保持为 0.12, 0.03, 0.03（可按需微调）
            # [1.3, 0.80, 0.435, 1.0, 0.0, 0.0, 0.0, 0.12, 0.03, 0.03]
            [1.3, 0.80, 0.435, 1.0, 0.0, 0.0, 0.0, 0.12, 0.10, 0.03]
            # [1.23, 0.80, 0.435, 1.0, 0.0, 0.0, 0.0, 0.12, 0.03, 0.03]
        ]
        ...
        super(FrankaFetchPickDynFrontUStaticGrooveEnv, self).__init__(
            model_path=model_path, n_substeps=n_substeps, n_actions=8,
            initial_qpos=initial_qpos)
    ...
    def step(self, action):
        ...
        return super(FrankaFetchPickDynFrontUStaticGrooveEnv, self).step(action)
    ...
    def _get_obs(self):
        ...
        # obstacle 2 （U形整体）
        # 虽然拆成多块geom，但依旧是一个 body：obstacle2
    ...
    def _reset_sim(self):
        ...
        # 为了保持第二个障碍（U形）大多时候不动，可手动将其速度降为极小值
    ...
    def render(self, mode='human', width=1080, height=1080):
        ...
        return super(FrankaFetchPickDynFrontUStaticGrooveEnv, self).render(mode, width, height)
